...
(function(){lightBot.ui.editor={saveProgram:function(){localStorage.setItem("lightbot_program_level_"+lightBot.map.getLevelNumber(),$("#programContainer ul").html())},loadProgram:function(){$("#programContainer ul").append(localStorage.getItem("lightbot_program_level_"+lightBot.map.getLevelNumber())).find(".ui-state-hover").removeClass("ui-state-hover")},makeDroppable:function(){$("#programContainer ul").droppable({greedy:"true",activeClass:"ui-state-droppable",hoverClass:"ui-state-droppable-hover",
accept:":not(.ui-sortable-helper)",drop:function(a,b){$(this).children(".placeholder").remove();var d=$(b.draggable.clone()).removeClass("ui-draggable");d.appendTo(this);d.children("div").hasClass("droppable")&&lightBot.ui.editor.makeDroppable();d=$(this).parent();d.parent().is("#programContainer")&&d.animate({scrollTop:d.height()},"slow");lightBot.ui.editor.saveProgram()}}).sortable({items:"li:not(.placeholder)",placeholder:"ui-state-highlight",cursor:"move",sort:function(){$("#programContainer ul").removeClass("ui-state-droppable")},
stop:function(){lightBot.ui.editor.saveProgram()}})},getInstructions:function(a){var b=[];a.each(function(){switch($(this).children("p").attr("class")){case "walk":b.push(new lightBot.bot.instructions.WalkInstruction);break;case "jump":b.push(new lightBot.bot.instructions.JumpInstruction);break;case "light":b.push(new lightBot.bot.instructions.LightInstruction);break;case "turnLeft":b.push(new lightBot.bot.instructions.TurnLeftInstruction);break;case "turnRight":b.push(new lightBot.bot.instructions.TurnRightInstruction);
break;case "repeat":var a=$(this).children("p").children("span").children("input").val(),c=lightBot.ui.editor.getInstructions($(this).children("div").children("div").children("ul").children("li"));b.push(new lightBot.bot.instructions.RepeatInstruction(a,c))}});return b}}})();var canvasView=function(a){lightBot.ctx=a.get(0).getContext("2d");window.setInterval(function(){if(lightBot.bot.isInExecutionMode()&&lightBot.bot.isReadyForNextInstruction()&&lightBot.bot.hasNextInstruction()){var a=$.extend({},lightBot.bot.currentPos),b=lightBot.bot.executeNextInstruction();lightBot.bot.animate(b,a,lightBot.bot.currentPos)}lightBot.map.ready()&&lightBot.map.state.check(lightBot.map.state.allLightsOn)&&(lightBot.bot.clearExecutionQueue(),a=lightBot.medals.awardMedal(),lightBot.medals.display(a),
